...
const CreateTravelOrderScreen = () => {
    ...
    const fetchTransportations = async () => {
      try {
        ...
        const response = await axios.get(
          `${BASE_URL}/travel-orders/transportations`,
          {
            headers: {
  ...
  return (
    <View style={styles.container}>
      ...
      />
      {errors.destination && (
        <Text style={styles.error}>{errors.destination[0]}</Text>
      )}
      <RNPickerSelect
      ...
      {errors.transportation_id && (
        <Text style={styles.error}>{errors.transportation_id[0]}</Text>
      )}
      <TextInput
        style={styles.input}
      ...
      />
      {errors.expenses && (
        <Text style={styles.error}>{errors.expenses[0]}</Text>
      )}
      <TextInput
        style={styles.input}
        placeholder="Purpose"
      ...
      <View>
        <Button
          title="Pick Date Leave"
          onPress={() => setShowDateLeavePicker(true)}
        />
        {showDateLeavePicker && (
          <DateTimePicker
            value={dateLeave}
      ...
      <View>
        <Button
          title="Pick Date Return"
          onPress={() => setShowDateReturnPicker(true)}
        />
